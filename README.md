...
- **Dual Authentication**:
  ...
  - **Telegram**: Scan QR code via *Telegram > Settings > Devices > Link Desktop Device* or authenticate via phone verification code with optional Cloud 2FA password.
...
async function main() {
  ...
  console.log(`WhatsApp state: ${status.status} (${status.phoneNumber || 'Unpaired'})`);

  // Send a text message
  const res = await whatsappBot.messages.sendText('201012345678', 'Hello from WhatsApp via Botcast SDK!');
...
async function main() {
  ...
  const res = await telegramBot.messages.sendText('+1234567890', 'Hello from Telegram MTProto!');
...
client.webhook.onMessage(async (event) => {
  console.log(`Message from ${event.sender}: ${event.message} [Platform: ${event.platform || 'whatsapp'}]`);
